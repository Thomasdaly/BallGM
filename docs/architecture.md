...
- **Cadence.** Two different answers, because the two systems started in different states. Draft selection was already unconditional and fully automatic for every team (no draft-day UI exists — see "Draft decisions" below), so `DraftDay` now calls `DraftDecisionModel.Recommend` for every selection, every season, same as before. Trade and signing execution is **on-demand only** — `RunAiFrontOfficeTurn` is a method a caller invokes, not something `ConcludeSeason` or any other season-boundary flow calls for itself. Wiring it into the season boundary is future work, once team control is actually decided; stacking unattended trade/signing execution onto a boundary that already runs unattended (`AutoResignToRosterFloor`, `DraftDay`) before that question is settled would be new scope, not glue.
...
No ruleset field and no save-schema change. `RunAiFrontOfficeTurn` is the one new public method on `LeagueSession` this slice adds, called out under `CLAUDE.md`'s change-control list; `DraftDay.Run`'s signature grew two parameters, both threaded from state `RulesSeasonEngine.RunDraft` already held, with its one caller updated alongside it. Still open, unchanged by this slice: a real "which team is mine" concept and any save-schema it would need; wiring `RunAiFrontOfficeTurn` into the season boundary; ranking several candidates instead of taking the first; and the inbox/news-feed UI the roadmap already names separately.
