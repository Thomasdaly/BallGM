...
**Draft classes, scouting uncertainty, and the lottery landed first** (ruleset schema version 7) — see `docs/architecture.md` → "Draft classes, scouting, and the lottery" for what shipped and the decisions worth not re-deriving. **Development, ageing, and retirement landed second** (ruleset schema version 8) — see `docs/architecture.md` → "Development, ageing, and retirement", including why the curve moves `PlayerRating.Overall` rather than a per-attribute breakdown. **Season stats, biography, and the award set landed third** (ruleset schema version 9) — see `docs/architecture.md` → "Records, history, and awards", including why career statistics (across more than one season) are a recorded gap rather than built here, and why an award is a stat leaderboard rather than a genuine vote until Milestone 9's AI front offices exist. Milestone 8's own list is now fully covered by what has shipped or is explicitly named as deferred; none of it is wired into `LeagueSession` or the client yet.
