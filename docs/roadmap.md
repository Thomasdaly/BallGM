...
All four of Milestone 9's roadmap-listed consumer systems — trade targeting, free-agent targeting, draft decisions, plus the organisational-direction/asset-valuation/roster-needs foundation under "explainable decisions and diagnostics" — are now built as pure, non-mutating Rules-layer models with no port and no `LeagueSession` wiring. What is still open from the roadmap's own Milestone 9 list: wiring any of the four into `LeagueSession`/a port/the client so an actual AI turn runs; cash as a tradeable asset; configurable participant/asset trade caps; contract buyouts and the post-buyout market; player signing demands; league power rankings; the daily offseason digest; and the naive placeholders (`AutoResignToRosterFloor`, `DraftDay`'s best-prospect-left selection) this milestone was always going to eventually replace.
