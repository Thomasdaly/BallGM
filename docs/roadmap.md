...
What is still open from the roadmap's own Milestone 9 list: whether/how an AI turn ever executes a trade or a signing unattended (which teams are AI-controlled, concurrency with the human GM's own actions, whether it runs every season boundary or on demand — a separate, later decision this slice deliberately did not make); cash as a tradeable asset; configurable participant/asset trade caps; contract buyouts and the post-buyout market; player signing demands; league power rankings; the daily offseason digest; and the naive placeholders (`AutoResignToRosterFloor`, `DraftDay`'s best-prospect-left selection) this milestone was always going to eventually replace.
