...
As of this writing: Milestones 0 (repo/architecture proof), 1 (league and roster foundation), 2 (thin playable UI slice), 3 (contracts and cap ledger), 4 (draft assets), 5 (trade engine), 6a (contract offers and signing routes), 6b (the free-agency market), 7a (the season calendar, schedule, standings, depth charts and postseason) and 7b (the match engine) are done. **Milestone 8 — player lifecycle — is now fully covered** by what has shipped or is explicitly named as a deferred/follow-up item; see the three slices below. The one piece of 7 that was still outstanding, **UI box scores**, is now done: the calendar screen (`SeasonView`) carries a "recent results" strip built from `LeagueSession.Schedule` (looking behind the current day) and a selected-game box score panel built from `LeagueSession.BoxScore`, two columns by team, ordered starters-then-minutes the way `BoxScore.LinesFor` already does. What's still not wired into `LeagueSession` or the client is Milestone 8's own three slices — draft classes/scouting/lottery, development/ageing/retirement, records/stats/awards — none of which has a client surface yet.

What Milestone 8 added, in three slices, at the time none of it was wired into `LeagueSession` or the client: (1) draft classes/scouting/lottery — `Prospect`/`DraftClass`/`ScoutingRange` in `BallGM.Domain.Draft`; `DraftClassRules`/`ScoutingRules`/`DraftLotteryRules` in `BallGM.Rules.Configuration`; `ProspectGenerator`/`ScoutingModel`/`DraftLottery`/`ProspectNameBank` in `BallGM.Rules.Draft` — ruleset schema version 7, and `DraftLottery.Run` is the `DraftOrderSnapshot` producer `docs/domain-language.md` named as owed since Milestone 4; (2) development/ageing/retirement — `Player.Develop`/`Retire`/`Biography` and `PlayerRating.Adjust`, `PlayerBiography` in `BallGM.Domain.Players`; `DevelopmentRules`/`RetirementRules` in `BallGM.Rules.Configuration`; `PlayerDevelopmentModel`/`RetirementModel` in `BallGM.Rules.Development` — ruleset schema version 8; (3) records/history/awards — `PlayerSeasonStatLine` in `BallGM.Domain.Seasons`; `AwardRules` in `BallGM.Rules.Configuration`; `PlayerSeasonStatsCalculator`/`SeasonAwardCalculator` in `BallGM.Rules.Seasons` — ruleset schema version 9. See `docs/architecture.md` → "Draft classes, scouting, and the lottery", "Development, ageing, and retirement", and "Records, history, and awards" for what shipped, what's deliberately deferred (a tracked scouting-investment economy, threading development/retirement into the season boundary, per-attribute development curves now that `PlayerRating` actually has attributes to curve, career statistics across more than one season, and a genuinely voted rather than leaderboard-decided award, which needs Milestone 9's AI front offices), and the decisions worth not re-deriving.

**Slice (1) is now wired to the season boundary.** `BallGM.Rules.Draft.DraftDay` orchestrates `ProspectGenerator` + `DraftLottery` into an actual draft — best prospect left to each selection in order, a traded pick routed to its current owner via `DraftAssetBook` — behind a new `ISeasonEngine.RunDraft` port, and `LeagueSession.ConcludeSeason` calls it automatically every season, right before `AutoResignToRosterFloor`. `data/rulesets/default-league.json` now states a `draftClass` section so the shipped content pack actually exercises it. A selection a full roster cannot take (there is no "draft rights" or roster-cut concept yet) is reported via `SeasonConclusionSummary.PlayersDraftedButUnrostered` and left as a plain free agent. See `docs/architecture.md` → "Draft classes, scouting, and the lottery" for the decisions worth not re-deriving. Slices (2) and (3) are still not wired to anything.
...
Calibration is locked by seeded regression tests in `MatchModelCalibrationTests`, so a tuning change that breaks the sport fails a test rather than quietly shipping. Two known limitations are recorded there, both because the match engine reads only a single derived `Overall` int — leading scorers and rotation minutes are both flatter than a real league's. `PlayerRating` itself is no longer single-attribute (see below), but the match engine was deliberately not touched to fix this: doing so means recalibrating the locked tests, a larger step than the rating expansion itself. Do not try to fix these two by pushing harder on `Overall`, which would make a great defensive centre his team's leading scorer.

**`PlayerRating` is now multi-attribute** (`Height`/`Speed`/`Strength`/`Passing`/`LateralQuickness`; `Overall` is derived, never stored) — see `docs/architecture.md` → "Draft classes, scouting, and the lottery" for `RatingProfileGenerator`, the empirically-tuned spread constant, and why the match engine and development curve were deliberately left untouched. Save schema moved 1 → 2 for this (`PlayerEnvelope` stores the five attributes); a version-1 save is refused, not migrated.

**The box score gained a real shooting line, in two steps.** First, exposure only: `PlayerStatLine` gained `OffensiveRebounds`/`DefensiveRebounds` and `UsagePercent`, both quantities `PossessionMatchEngine.Side` already computed internally and either merged or discarded — no new randomness, no recalibration needed or done. Second, a genuinely new mechanic: `PlayPeriod` no longer decides a possession with one binary score/miss draw — it now draws whether a shot happens at all, a shot type, an efficiency-adjusted field-goal percentage, and (on a miss or make) a shooting foul or and-one running a real free-throw trip. `PlayerStatLine` gained FGA/FGM/3PA/3PM/FTA/FTM, with `Points == 2×(FGM−3PM)+3×3PM+FTM` enforced at construction. `BaseOffensiveEfficiency` retuned 10,800 → 11,500 (the sim audit's own `ortg` target). A new `MatchModelBounds.GameShootingVarianceRange` — a per-game correlated "hot/cold night" term — was needed to hit `sd_game_margin`: splitting one draw into several smaller ones measured as *reducing* game-to-game scoring variance, the opposite of what the audit's own finding suggested, because real shooting variance is correlated within a game rather than shot-to-shot independent. Two `MatchModelCalibrationTests` bands moved as a result (documented in that file as the retune itself, not a convenience widening) — see `docs/architecture.md` → "Real shot attempts, free throws, and the ortg/margin retune" for the full reasoning. Turnovers/steals/blocks/fouls remain entirely unmodeled — not needed by anything in `.claude/skills/sim-regress/suite.md`. Save schema moved 2 → 3 for this.

**All 12 rows of the frozen sim-regress suite now pass** (`.claude/skills/sim-regress/suite.md`). The last one, `noll_scully`, needed its own small nudge: the target formula's `sqrt(games)` term defaults to a real-NBA 82-game season, but this league plays 78 (`data/rulesets/default-league.json`'s `regularSeasonGameCount`) — using the correct denominator, `sd_team_win_pct` alone (already in band) put `noll_scully` at 2.491, a hair under its 2.50 floor. `TeamStrengthOffsets` was nudged once more (`[14,6,0,-4,-11,-18]` → `[15,6,0,-4,-12,-19]`) to clear it with real margin: measured sd_team_win_pct ≈ 0.147, noll_scully ≈ 2.60, talent_share_of_var ≈ 0.85, all three in band together. Everything past the frozen 12 is the audit's fuller wishlist, not gated by anything: turnovers/steals/blocks/fouls, and battery 4's attribute↔stat couplings/usage-stacking slopes (need a match-engine multi-attribute wiring pass, deliberately deferred in P-3).
