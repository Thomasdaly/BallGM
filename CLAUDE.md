...
As of this writing: Milestones 0 (repo/architecture proof), 1 (league and roster foundation), 2 (thin playable UI slice), 3 (contracts and cap ledger), 4 (draft assets), 5 (trade engine), and **6a** (contract offers and signing routes) are done. Milestone 6b — the free-agency *market* — is next, and it is the half of Milestone 6 that carries the open design questions: the `Negotiation` state machine with its offer history, `PlayerPreference` decomposed per factor, seeded tie-breaking, simultaneity and resolution ordering, competing offers, and the positional free-agency board.
