...
As of this writing: Milestones 0 (repo/architecture proof), 1 (league and roster foundation), 2 (thin playable UI slice), 3 (contracts and cap ledger), 4 (draft assets), 5 (trade engine), 6a (contract offers and signing routes), 6b (the free-agency market), and **7a** (the season calendar, schedule, standings, depth charts and postseason) are done. Milestone 7b — the match engine that actually decides a game — is next: `IMatchEngine` exists and the only implementation this build ships is `UnplayedMatchEngine`, which plays nothing and says so.
